...
import org.d3if3034.miniproject.util.SettingsDataStore
                            ...
                            DropdownMenuItem(
                                leadingIcon = {
                                    Icon(
                                        ...
                                        contentDescription = stringResource(R.string.newton),
                                        tint = MaterialTheme.colorScheme.primary
                                    )
                                },
                                text = {
                                    Text(stringResource(R.string.newton))
                                },
                                onClick = {
                                    expanded = false
                                    navController.navigate(Screen.Hitung.route)
                                }
                            )
                        }
                    }
        ...
        floatingActionButton = {
            FloatingActionButton(
                ...
                Icon(
                    ...
                    contentDescription = stringResource(R.string.tambah_praktikum),
                    tint = MaterialTheme.colorScheme.primary
                )
            }
        }
    ...
    if (data.isEmpty()) {
        ...
        if (showList) {
            ...
            LazyVerticalStaggeredGrid(
                ...
                contentPadding = PaddingValues(10.dp, 10.dp, 10.dp, 94.dp)
            ) {
                items(data) {
                    GridItem( praktikum = it) {
                        navController.navigate(Screen.FormUbah.withId(it.id))
                    }
                }
            }
...
@Composable
fun ListItem(praktikum: Praktikum, onClick: () -> Unit) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() }
        ...
        Text(text = praktikum.judul,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = FontWeight.Bold
        )
        Text(text = praktikum.lab,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis
        )
        Text(text = praktikum.rangkuman,
            maxLines = 5,
            overflow = TextOverflow.Ellipsis
        )
        Text(text = praktikum.tanggal)
    }
}

@Composable
fun GridItem(praktikum: Praktikum, onClick: () -> Unit) {
    Card (
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() },
